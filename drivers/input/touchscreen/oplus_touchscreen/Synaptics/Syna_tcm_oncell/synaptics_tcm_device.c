...
static inline void device_capture_touch_report(unsigned int count)
{
    int retval;
    unsigned char id;
    unsigned int idx;
...
static inline int device_capture_touch_report_config(unsigned int count)
{
    int retval;
    unsigned int size;
    unsigned char *data;
...
static inline long device_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
#else
static inline int device_ioctl(struct inode *inp, struct file *filp, unsigned int cmd,
                        unsigned long arg)
#endif
{
    int retval = 0;
...
static inline loff_t device_llseek(struct file *filp, loff_t off, int whence)
{
    return -EINVAL;
}

static inline ssize_t device_read(struct file *filp, char __user *buf,
...
static inline ssize_t device_write(struct file *filp, const char __user *buf,
                            size_t count, loff_t *f_pos)
{
    int retval;
    struct syna_tcm_data *tcm_info = g_device_hcd->tcm_info;
...
static inline int device_open(struct inode *inp, struct file *filp)
{
    int retval;
...
static inline int device_release(struct inode *inp, struct file *filp)
{
...
static inline char *device_devnode(struct device *dev, umode_t *mode)
{
    if (!mode)
        return NULL;
...
static inline int device_create_class(void)
{
...
static inline int device_init(struct syna_tcm_data *tcm_info)
{
    int retval;
    dev_t dev_num;
...
inline int syna_remote_device_destory(struct syna_tcm_data *tcm_info)
{
    if (!g_device_hcd)
        return 0;
