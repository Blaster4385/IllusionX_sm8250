...
struct touchpanel_data {
	...
	struct kernel_grip_info
		*grip_info;                 /*grip setting and resources*/
...
struct oplus_touchpanel_operations {
	...
	int (*cs_gpio_control)(void *chip_data,
			       bool enable);                              /*used for cs gpio*/
...
__attribute__((weak)) int opticalfp_irq_handler_uff(struct fp_underscreen_info
		*fp_tpinfo)
{
	return 0;
}
