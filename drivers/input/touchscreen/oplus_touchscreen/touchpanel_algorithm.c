...
static void init_touch_point(struct algo_point_buf *point_buf,
...
static void init_kalman_point(struct algo_point_buf *point_buf,
			      struct point_info *point)
{
		...
		if (point_buf->status == CLICK_UP) {
			...
			if (ts->pressure_report_support) {
				input_report_abs(ts->input_dev, ABS_MT_PRESSURE,
						 point_buf->down_point.z);   //add for fixing gripview tap no function issue
			}
			 ...
			 uint16_t delta_time, struct algo_kalman_info *kal_value)
{
	...
	x_now = (x_now + (long)(p_mid * (ResrcData - x_mid)) / (p_mid + R)) /
		VARIABLE_GAIN;
...
static void kalman_handle(struct touchpanel_data *ts, int obj_attention,
			  struct point_info *points)
{
	...
	for (i = 0; i < ts->max_num; i++) {
		...
		if (((obj_attention & TOUCH_BIT_CHECK) >> i) & 0x01
			...
			KalmanFilter(points[i].x, &point_buf->kal_x_last.x, &point_buf->kal_p_last.x,
				     point_buf->delta_time, algo_info->kalman_info_used);
			KalmanFilter(points[i].y, &point_buf->kal_x_last.y, &point_buf->kal_p_last.y,
				     point_buf->delta_time, algo_info->kalman_info_used);
...
static int stretch_point_handle(struct touchpanel_data *ts, int obj_attention,
				struct point_info *points)
{
	int i = 0;
	int obj_new = 0;
		...
		if (((obj_attention & TOUCH_BIT_CHECK) >> i) & 0x01
			...
			if (point_buf->touch_time == 0) {
				...
				if (algo_info->stretch_info.top_frame > 0
					...
					if (algo_info->phone_direction & algo_info->stretch_info.top_open_dir) {
						TPD_INFO("The id %d maybe need top stretch point is %d,%d", i, points[i].x,
							 points[i].y);
					...
					if (algo_info->phone_direction & algo_info->stretch_info.bottom_open_dir) {
						TPD_INFO("The id %d maybe need bottom stretch point is %d,%d", i, points[i].x,
							 points[i].y);
				...
				if (algo_info->stretch_info.left_frame > 0
					...
					if (algo_info->phone_direction & algo_info->stretch_info.left_open_dir) {
						TPD_INFO("The id %d maybe need left stretch point is %d,%d", i, points[i].x,
							 points[i].y);
					...
					if (algo_info->phone_direction & algo_info->stretch_info.right_open_dir) {
						TPD_INFO("The id %d maybe need right stretch point is %d,%d", i, points[i].x,
							 points[i].y);
			...
			if (point_buf->status & TOP_START) {
				if (point_buf->down_point.y < points[i].y
					...
					temp = ((temp * algo_info->stretch_info.top_stretch_time) /
						(point_buf->touch_time + 1)) + 1;
			...
			if (point_buf->status & BOTTOM_START) {
				if (point_buf->down_point.y  > points[i].y
					...
					temp = ((temp * algo_info->stretch_info.bottom_stretch_time) /
						(point_buf->touch_time + 1)) + 1;
					points[i].y = point_buf->down_point.y + temp;
			...
			if (point_buf->status & LEFT_START) {
				if (point_buf->down_point.x  < points[i].x
					...
					temp = ((temp * algo_info->stretch_info.left_stretch_time) /
						(point_buf->touch_time + 1)) + 1;
			...
			if (point_buf->status & RIGHT_START) {
				if (point_buf->down_point.x  > points[i].x
					...
					temp = ((temp * algo_info->stretch_info.right_stretch_time) /
						(point_buf->touch_time + 1)) + 1;
					points[i].x = point_buf->down_point.x + temp;
...
void switch_kalman_fun(struct touchpanel_data *ts, bool game_switch)
{
	...
	if (game_switch) {
		...
		if (CHK_BIT(ts->algo_info->kalman_info_used->enable_bit,
				(1 << ts->smooth_level)))
			ts->algo_info->enable_kalman = true;
				       ...
				       const char __user *buf, size_t count, loff_t *lo)
{
	...
	if (ts->algo_info->charger_kalman_info.Q_Min >
			ts->algo_info->charger_kalman_info.Q_Max)
		ts->algo_info->charger_kalman_info.Q_Max =
			ts->algo_info->charger_kalman_info.Q_Min;
...
void oplus_touch_algorithm_init(struct touchpanel_data *ts)
{
	...
	if (algo_info->charger_kalman_info.Q_Min >
	...
	algo_info->point_buf = kzalloc(sizeof(struct algo_point_buf) * ts->max_num,
				       GFP_KERNEL);
	...
	prEntry_tmp = proc_create_data("stretch_config_set", 0666, ts->prEntry_tp,
				       &proc_stretch_config_ops, ts);

	if (prEntry_tmp == NULL)
		TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);

	prEntry_tmp = proc_create_data("kalman_value_set", 0666, ts->prEntry_tp,
				       &proc_kalman_value_ops, ts);
